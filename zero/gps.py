...
class Gps ():
  ...
  def gps_status (self):
    ...
    # writing gps log
    self.buoy.track (str(self.gps_time) + "," + ("1" if self.valid else "0") + "," + str(self.latitude) + "," + ('N' if self.north else 'S') + ", " + str(self.longitude) + "," + ('E' if self.east else 'W') + "," + ('1' if self.has_time else '0') + "," + ('1' if self.has_sync else '0') + "," + ('1' if self.has_sync_reference else '0'))
